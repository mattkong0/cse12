...
class Maze {
	...
	public ArrayList<Square> storePath() {
		ArrayList<Square> pathToFinish = new ArrayList<Square>();
		Square current = this.finish;
		pathToFinish.add(current);
		while (current.getPrevious() != null) {
			pathToFinish.add(0, current.getPrevious()); // add to front
			current = current.getPrevious(); // set current to previous
		}
		if (pathToFinish.get(0) != this.start || 
			pathToFinish.get(pathToFinish.size()) != this.finish) {// if there's no path
			ArrayList<Square> blankPath = new ArrayList<Square>(); // make new blank path
			return blankPath; // return a blank path
		}
		return pathToFinish;
	}
}
